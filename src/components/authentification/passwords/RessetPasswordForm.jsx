...
function ResetPasswordForm() {
  ...
  const handleSubmit = (event) => {
    ...
    .then((res) => {
      if (res.ok) {
        setFlashMessage('Un e-mail contenant les instructions de réinitialisation de votre mot de passe a été envoyé.');
        return res.json();
      } else {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <label htmlFor="email" className='wdConnexion'>Adresse e-mail :</label>
        <br />
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className='inputF' required />
        <br />
        <br />
